...
const InteractionBtn = ({ mbid, rating: initialRating, favorite, queue }) => {
  ...
  const StarIcon = ({ value = 0 }) => {
    ...
    return (
      <svg
        ...
        style={{
          filter:
            value > 0 ? "drop-shadow(0 0 5px rgba(251,191,36,0.65))" : "none",
      ...
      >
        <defs>
          <linearGradient
            ...
            y2="0%"
          >
            <stop offset={`${fillPercent}%`} stopColor="rgb(251,191,36)" />
  ...
  const handleRate = async (val) => {
    try {
      const newVal = rating === val ? 0.0 : val;
    ...
    <div className="flex flex-col gap-5 w-full max-w-md">
      ...
      <div className="flex flex-col gap-2 lg:w-150">
        <div className="flex flex-col gap-2 pt-4 border-t border-white/10">
          ...
          >
            {[1, 2, 3, 4, 5].map((star) => {
              const currentRating = hovered || rating || 0;

              const starValue = Math.max(
                0,
                Math.min(1, currentRating - star + 1),
              );
      ...
      {rating !== 0 ? (
        <div className="flex flex-col gap-2 lg:w-150">
          <textarea
            ...
            disabled={rating === 0}
            placeholder={
              rating > 0 ? `Write your thoughts` : `rate me first :)`
            }
          ...
          {rating > !0 ? (
            ...
            <div className="transition-all ease-in-out duration-1000">
              <p
                className={`text-xs transition-opacity duration-500 text-gray-400`}
              >
        ...
        <div className="lg:flex justify-between mb-2">
          ...
          {last3Reviews.length >= 3 && (
            <button
              ...
              className
            >
              <h1 className="text-xs pb-px cursor-pointer border-b">
                OPEN ALL REVIEWS
              </h1>
            </button>
          )}
